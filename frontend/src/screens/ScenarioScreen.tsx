          ...
          <div className="space-y-2.5">
            {scenarios.map((s) => (
              <button
                ...
                className="w-full rounded-lg border border-line bg-slate-50 p-3 text-left transition hover:border-accent hover:bg-accent-soft"
              >
                <div className="text-sm font-semibold text-slate-900">{s.title}</div>
                <div className="mt-1 text-sm leading-snug text-slate-500">{s.description}</div>
              </button>
            ))}
          </div>
        </div>
